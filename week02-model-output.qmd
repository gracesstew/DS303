...
-   Expressed as ***y*** **= *b~0~* + *b~1~x~1~* + *b~2~x~2~* + … *b~n~x~n~***
...
-   ***intercept***: When all coeff. are zero, median home value is 41.62.

-   ***crim***: per capita rate by town

    -   P value is close to 0, meaning it is statistically significant. With each one unit increase in crim, the estimate median value decreases by \~0.12 units.

-   ***indus***: proportion of non-retail business acres per town

    -   P value is closer to 1, meaning it is NOT statistically significant. With each one unit increase in indus, the estimate median value increases by \~0.013 units; however, were it removed from the model, the difference would be insignificant.

-   ***age***: proportion of owner-occupied units built prior to 1940

    -   P value is closer to 1, meaning it is NOT statistically significant. With each one unit increase in age, the estimate median value increases by \~0.004 units; however, were it removed from the model, the difference would be highly insignificant.

**Model Fit**:

**Do you think this is a good model to use for predicting median home values in Boston?**

-   Overall, the multiple R-squared is \~0.734. This means that roughly 73.4% of variance in the model is explained by the predictors in the model. There could be more that can be done to improve the model, but all-in-all the model fit well.
