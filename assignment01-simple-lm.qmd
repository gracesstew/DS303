...
-   *y* = *b~0~* + *b~1~x* and *y* = *mx* + *b* are essentially the same equation; however, the context in which we use them differs. *y* = *b~0~* + *b~1~x* is used in statistical situations in which we are focused on exploring parameters of data and making inferences from their relationships, whereas in *y* = *mx* + *b* we are focused on more mathematical contexts in which we are graphing relationships and visualizing through slopes and intercepts.
...
-   The model output tell you the intercepts and slope. With every 1 unit increase in rm, medv increases by 9.1 model units.
...
-   From this summary, you get the P value (how significant a variable is to the model), R-squared (how well the model explains any variability present), and residuals (which helps explain overall model fit and prediction error).
...
-   Confint() calculates confidence interval. Predict() generates predicted values for new data.
...
-   A variable I am interested in is crim (per capita crime rate by town) in relation to medv (median value of owner-occupied homes in \$1000s).
...
-   Overall, the crime rate has a statistically significant relationship with home value (due to its extremely low p-value). With every 1 unit increase in crim, there is a \$0.42k *decrease* in medv. This model shows that crime rate explains about 15% of the variation in home values.
