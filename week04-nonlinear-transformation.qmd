...
### Fit the model with a quadratic term (squared term)
...
### low p-value w lots of stars: tells us the models are significantly different
### we want the lower RSS
### model 2 is better
### F=135, reduction in residual error is 135x larger than what itd be from random noise
```
...
## I(lstat^2) tells us that the curve slows bc its positive
```
...
##more u add, more complex interpretation becomes
## singular
## quad
## cubic
```
...
```{r}
lm.fit_higher5 <- lm(medv ~ poly(lstat, 5), data = Boston)
lm.fit_higher10 <- lm(medv ~ poly(lstat, 10), data = Boston)

anova(lm.fit3, lm.fit_higher5, lm.fit_higher10)

ggplot(Boston, aes(x = lstat, y = medv)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, 10))

### stars = best model, 10 (higher) not necessarily better
```
...
### quadratic seems like it would fit this data best
```
...
lm.crim2 <- lm(medv ~ poly(crim, 2), data=Boston)
summary(lm.crim2)

anova(lm.crim, lm.crim2)
