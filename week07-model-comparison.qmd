...
ANOVA stands for **AN**alysis **O**f **VA**riance. ANOVA is traditionally used to compare means across different groups to see if there's any statistically significant difference between them. However, when it comes to model comparison, ANOVA can be interpreted as performing a **likelihood ratio test.**
...
Typically you will need the models you are comparing to be **nested** in structure. **Nested models** refer to two models where one is a simpler version of the other.
...
-   `Sum of Sq` = This is the **reduction in unexplained variance** when adding a predictor.\
    (278.32 − 195.05 = 83.274)
...
# in order to use "forward", you need to present it a "scope" by including the null model
m.null <- lm(mpg ~ 1, data = mtcars)
stepwise_model <- step(m.null, scope = list(lower = m.null, upper = m.all), direction = "forward")
```
...
m <- cor(mtcars)
corrplot(m)
corrplot(abs(m)) # absolute value of m, so everything is positive
```
